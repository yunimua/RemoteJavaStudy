...
public class Student implements Comparable<Student>{
	...
	final private int NUM_OF_SUVJECT = 3; // 과목 3개
	
	private static int curr_snum = 1000;
	private static Random ran = new Random();
	
	public Student() {
		snum = curr_snum++;  //1000번부터 학번 생성
	...
	public int compareTo(Student o) {
		//return (int) Math.ceil(this.avg - o.avg); //+ 0.1이라도 차이가 나면 올려라!
			
		return Double.compare(this.avg, o.avg) != 0 ? 
				Double.compare(this.avg, o.avg) : Integer.compare(snum, o.snum); //+ 0이 아니라면 그대로 정렬
	}
}
